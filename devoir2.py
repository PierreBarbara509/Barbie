...
chenn_miniskil = ''.join(char.lower() for char in chenn)
...
chenn = "Ayiti"
chenn_modifikasyon = (chenn + chenn).upper()
print(chenn_modifikasyon)

#Afiche endeks premye karaktè "a" nan yon chenn:
chenn = "Ayiti kapab avanse"
endeks = next((i for i, char in enumerate(chenn) if char == 'a'), -1)
print(endeks) 

#Afiche total tout endeks karaktè "a" nan yon chenn:
chenn = "Ayiti kapab avanse"
total_endeks = len([i for i, char in enumerate(chenn) if char == 'a'])
print(total_endeks)

#Kreye yon lis ki gen endeks tout karaktè "a" nan yon chenn (sèlman a miniskil):
chenn = "Ayiti kapab avanse"
endeks_miniskil = [i for i, char in enumerate(chenn) if char == 'a' and chenn[i].islower()]
...
chenn_sans_espas = ''.join(chenn.split())
kantite_karaktè = len(chenn_sans_espas)
print(chenn_sans_espas, kantite_karakte)
